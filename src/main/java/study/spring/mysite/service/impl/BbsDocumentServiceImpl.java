...
public class BbsDocumentServiceImpl implements BbsDocumentService {
	...
	/**@Override
	public void updateLike(BbsDocument document) throws Exception {
		try {
			int result = sqlSession.update("BbsDocumentMapper.updateLike", document);
			if (result == 0) {
				throw new NullPointerException();
			}
		} catch (NullPointerException e) {
			...
			throw new Exception("존재하지 않는 게시물에 대한 요청입니다.");
		 	...
		 	// sqlSession.rollback();
			logger.error(e.getLocalizedMessage());
			throw new Exception("좋아요 갱신에 실패했습니다.");
		} finally {
			// sqlSession.commit();
		}
	}*/
	...
	@Override
	public List<BbsDocument> selectDocumentListMypage(BbsDocument document) throws Exception { //mypage
		List<BbsDocument> result = null;
		try {
			result = sqlSession.selectList("BbsDocumentMypageMapper.selectDocumentMypageList", document);
			if (result == null) {
				throw new NullPointerException();
			}
		} catch (NullPointerException e) {
	...
	@Override
	public int selectDocumentCountMypage(BbsDocument document) throws Exception { //mypage
		int result = 0;
		try {
			// 게시물 수가 0건인 경우도 있으므로, 
			// 결과값이 0인 경우에 대한 예외를 발생시키지 않는다.
			result = sqlSession.selectOne("BbsDocumentMypageMapper.selectDocumentMypageCount", document);
		} catch (Exception e) {
			logger.error(e.getLocalizedMessage());
			throw new Exception("게시물 수 조회에 실패했습니다.");
		}
