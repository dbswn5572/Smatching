...
@Controller
public class DocumentMypageList {
	...
	@Autowired
	UploadHelper upload;
	@RequestMapping(value = "/bbs/document_list_mypage.do")
	public ModelAndView doRun(Locale locale, Model model, HttpServletRequest request, HttpServletResponse response, int memberId, String wname) {
		...
		Member Info = null;
		try {
			Info = memberService.selectMember(member);
		} catch (Exception e) {
		...
		System.out.println(Info.getProfileImg());
		...
		if (profileImg != null) {
			try {
				String userprofileThumbnail = upload.createThumbnail(profileImg, 300, 300, true);
				web.setCookie("userprofileThumbnail", userprofileThumbnail, -1);
			} catch (Exception e) {
				return web.redirect(null, e.getLocalizedMessage());
			}
		}
		...
		try {
			...
			documentList = bbsDocumentService.selectDocumentListMypage(document);
		} catch (Exception e) {
			return web.redirect(null, e.getLocalizedMessage());
		}
